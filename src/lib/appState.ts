...
export const defaultJapanesePromptConfig: PromptConfig = {
  template: [
    ...
    '我会给你一个日语句子、上下文，以及已过滤标点的 token 列表。token index 保留原句中的原始位置。',
    '你的任务是基于 token 重组成适合阅读理解的语法语块，并用中文解释整句。',
    '',
    '重要规则：',
    '1. 必须只输出一个 JSON 对象，不要输出 Markdown 或额外说明。',
    '2. token_indices 必须使用输入 token 的原始 index，不允许自造或重排。',
    '3. 所有输入 token 必须被覆盖一次且仅一次；不要补回未输入的标点。',
    '4. 每个 chunk 必须等于 token_indices 对应 token.surface 直接拼接后的字符串。',
    '5. token_indices 按升序排列；每个语块只能覆盖连续原文 token。',
    '6. 助词、助动词、接尾词、补助动词与前后成分构成明确语法功能时，优先合并。',
    '',
    'JSON 结构固定为：',
    '{',
    '  "grammar": "string",',
    ...
    '要求：',
    '1. 必须使用中文回答。',
    '2. grammar 解释整句最重要的语法结构，不要只罗列词义。',
    '3. meaning 自然说明句意、语气和上下文作用。',
    '4. chunkAnalysis 按原句顺序返回语块；reading 使用平假名。',
    '5. grammar_role 用短中文说明语块功能。',
    '6. explanation 简要说明语块意思和语法功能。',
    '7. highlights 返回 0-4 个知识点；text 必须严格来自原句，kind 只能是 grammar、phrase、vocabulary。',
    '8. grammar 和 meaning 不要留空；不确定时不要编造。',
    '',
    '文档元信息：',
    '{documentMetadata}',
    '',
